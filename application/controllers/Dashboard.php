...
class Dashboard extends CI_Controller {
	...
	public function index()
	{
		...
		if (!isset($this->session->user['user_id'])) {//NOT SET YET
			...
			if ($logged_in = 1) {
				$data['user'] = json_decode(json_encode($this->users_model->setUser()),false);
				$data['sales']= json_decode(json_encode($data['sales']),false);
			}
		} else {//USER ALREADY SET
			$data['user']= json_decode(json_encode(
				$this->session->user
				),false);
			$data['sales'] = json_decode(json_encode(
				$this->sales_model->getSales()
				),false);
			$data['transactions'] = json_decode(json_encode(
